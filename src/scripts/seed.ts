...
import { IElection } from "../app/repositories/election.repo/election.entity";
...
const election_types = [
    { label: "AGM", type: ELECTION_TYPE.PREFERENCE },
    { label: "State Election", type: ELECTION_TYPE.PREFERENCE },
    { label: "Federal Election", type: ELECTION_TYPE.PREFERENCE },
    { label: "Referendum", type: ELECTION_TYPE.RANKING },
    { label: "Plebiscite", type: ELECTION_TYPE.YES_NO },
    { label: "Poll", type: ELECTION_TYPE.YES_NO }
]
...
export async function main() {
    ...
    for (let o = 0; o < 5; o++) {
        ...
        const elections = await promiseAll(randInt(5, 30), () => {
            const start = now + randInt(-5, 200) * ONE_DAY;

            const election = randElection()
        ...
        for (let en = voters.length * 5; en > 0; en--) {
            ...
            const election = randPick(elections);
            enrollmentRepo.getOrCreateEnrollment(voter.id, election.id)
        }

        for (let en = candidates.length * 5; en > 0; en--) {
            const candidate = randPick(candidates);
            const election = randPick(elections);
            runningRepo.getOrCreateRunning(candidate.id, election.id)
        }
